...
class MINDIterator(BaseIterator):
    ...
    def init_news(self, news_file):
        ...
        with tf.io.gfile.GFile(news_file, "r") as rd:
            for line in rd:
                ...
                #print("init_news","tokenized news_title",title) # ['best', 'sports', 'car', 'deals', 'for', ... ]
        ...
        for news_index in range(len(news_title)):
            ...
            for word_index in range(min(self.title_size, len(title))):
                ...
                #print("init news","word2index",self.news_title_index[news_index]) # init news word_dict [ 713  740  373  412    1  182 1158  339    0 .. ]
    ...
    #             # advised by https://zhuanlan.zhihu.com/p/404737215
    #             #impr_news = [self.nid2index[i.split("-")[0]] for i in impr.split()]
    #             #label = [int(i.split("-")[1]) for i in impr.split()]

    #             from random import choice
    #             if len(impr.split()) and impr.split()[0].find("-") == -1:
    #                 impr_news = [self.nid2index[i.split("-")[0]] for i in impr.split()]
    #                 label = [choice([0, 1]) for i in impr.split()]
    #             else: 
    #                 impr_news = [self.nid2index[i.split("-")[0]] for i in impr.split()]
    #                 label = [int(i.split("-")[1]) for i in impr.split()]
    ...
    def init_behaviors(self, behaviors_file):
        ...
        with tf.io.gfile.GFile(behaviors_file, "r") as rd:
            ...
            for line in rd:
                ...
                #print("init behaviors", uid, time, history, impr) # init behaviors U14167 11/11/2019 5:31:37 PM N118998 N55661 N83722 ..

                #print(line.strip("\n").split(self.col_spliter))

                history = [self.nid2index[i] for i in history.split()] # map to index
                history = [0] * (self.his_size - len(history)) + history[
                    : self.his_size
                ] # the maximum number of histories is self.his_size 
    ...
    def load_data_from_file(self, news_file, behavior_file):
        ...
        #print("begin load data:", hasattr(self, "news_title_index")) #False
        #print("begin load data:", hasattr(self, "impr_indexes")) #False

        if not hasattr(self, "news_title_index"):
            self.init_news(news_file)
        else:
            print("WARNING: The news_file is omitted")

        if not hasattr(self, "impr_indexes"):
            self.init_behaviors(behavior_file)
        else:
            print("WARNING: The behavior_file is omitted")
