...
class BaseModel:
    ...
    def __init__(
        ...
        self.eval_iterator = iterator_creator(
            hparams,
            col_spliter="\t",
        )
    ...
    def fit(
        ...
        for epoch in range(1, self.hparams.epochs + 1):
            ...
            eval_res = self.run_inference("eval", valid_news_file, valid_behaviors_file)
            eval_info = ", ".join(
                [
                    str(item[0]) + ":" + str(item[1])
            ...
            if test_news_file is not None:
                test_res = self.run_inference("test", test_news_file, test_behaviors_file)
                test_info = ", ".join(
                    [
                        str(item[0]) + ":" + str(item[1])
    ...
    def run_inference(self, flag, news_filename, behaviors_file):
        """Evaluate the given file and returns some evaluation metrics.

        Args:
            flag (str): "test" or tval"
        ...
        if self.support_quick_scoring:
            _, group_labels, group_preds = self.run_fast_eval(
                flag, news_filename, behaviors_file
            )
        else:
    ...
    def run_user(self, flag, news_filename, behaviors_file):
    ...
    def run_news(self, flag, news_filename):
    ...
    def run_slow_eval(self, flag, news_filename, behaviors_file):
        preds = []
        labels = []
        imp_indexes = []
    ...
    def run_fast_eval(self, flag, news_filename, behaviors_file):
        news_vecs = self.run_news(flag, news_filename)
        user_vecs = self.run_user(flag, news_filename, behaviors_file)
